-- =============================================================================
-- SCRIPT DDL: BASE DE DATOS MODULO GESTION DE INVENTARIOS (MER v3)
-- Proyecto de Titulo - Ingenieria Civil en Informatica (UBB)
-- Beneficiario: Sexta Compania de Bomberos de Chillan Viejo
-- Motor: PostgreSQL 16
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- 1. TABLAS MAESTRAS DE CLASIFICACION Y SEGURIDAD
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS USUARIO (
    ...
    id_voluntario INT NULL, -- Enlace logico con modulo de Gestion de Personal
...
CREATE TABLE IF NOT EXISTS TIPO_ITEM (
    ...
    tipo_clasificacion VARCHAR(50) NOT NULL UNIQUE -- 'AGRUPABLE_LOTE' vs 'UNITARIO_ETIQUETABLE'
);

CREATE TABLE IF NOT EXISTS TIPO_UBICACION (
    id_tipo_ubicacion SERIAL PRIMARY KEY,
    tipo VARCHAR(50) NOT NULL UNIQUE -- 'CARRO_BOMBA', 'BODEGA_CENTRAL', 'COMPARTIMENTO_CORTINA', 'ESTANTE', 'CABANA_PANOL'
);

CREATE TABLE IF NOT EXISTS TIPO_MOVIMIENTO (
    id_tipo_mov SERIAL PRIMARY KEY,
    tipo_mov VARCHAR(50) NOT NULL UNIQUE -- 'ALTA_COMPRA', 'ALTA_DONACION', 'TRASLADO', 'DEVOLUCION', 'EXTRAVIO_EMERGENCIA', 'BAJA_DETERIORO', 'RECUPERACION_POST_SINIESTRO', 'AJUSTE'
);

CREATE TABLE IF NOT EXISTS TIPO_INSPECCION (
    id_tipo_inspeccion SERIAL PRIMARY KEY,
    nombre VARCHAR(50) NOT NULL UNIQUE, -- 'RUTINARIA_PERIODICA', 'POST_EMERGENCIA'
    descripcion TEXT NULL
);

CREATE TABLE IF NOT EXISTS ESTADO_ALERTA (
    id_estado_alerta SERIAL PRIMARY KEY,
    nombre VARCHAR(50) NOT NULL UNIQUE, -- 'PENDIENTE', 'RESUELTA_HALLAZGO', 'CONFIRMADA_EXTRAVIO', 'TRAMITADA_BAJA', 'DESCARTADA'
    descripcion TEXT NULL
);

-- -----------------------------------------------------------------------------
-- 2. TABLAS PRINCIPALES: UBICACIONES E ITEMS
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS ITEM (
    ...
    codigo_qr VARCHAR(100) NULL UNIQUE, -- Nullable para items agrupables
    nombre VARCHAR(150) NOT NULL,
    descripcion TEXT NULL,
    estado VARCHAR(50) NOT NULL DEFAULT 'OPERATIVO', -- 'OPERATIVO', 'EN_MANTENCION', 'DANADO', 'DADO_DE_BAJA', 'EXTRAVIADO'
    cantidad INT NOT NULL DEFAULT 1 CHECK (cantidad >= 0),
    fecha_vencimiento DATE NULL, -- Caducidad perecibles o fecha proxima revision tecnica
...
-- -----------------------------------------------------------------------------
-- 3. ASIGNACION FISICA Y BALANCE DE STOCK ACTUAL
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. MOVIMIENTOS Y TRAZABILIDAD (LEDGER INMUTABLE)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 5. AUDITORIA, INSPECCIONES Y GESTION DE DISCREPANCIAS
-- -----------------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS ALERTA_DISCREPANCIA (
    ...
    id_usuario INT NULL REFERENCES USUARIO(id_usuario) ON UPDATE CASCADE ON DELETE SET NULL -- Oficial resolutor
);

-- -----------------------------------------------------------------------------
-- 6. DATOS SEMILLA INICIALES (SEED DATA PARA BOMBEROS)
-- -----------------------------------------------------------------------------
